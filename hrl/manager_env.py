...
class TableTennisManager(CustomEnv):
    """
    High-level manager controlling a frozen worker.
    """

    def __init__(self, worker_env: Any, worker_model: Any,
                 decision_interval: int = 10,
                 max_episode_steps: int = 800):
        super().__init__(worker_env.config)
    ...
    def step(self, action: np.ndarray):
        ...
        for _ in range(self.decision_interval):
            ...
            _, _, term, trunc, info = self.worker_env.step(a)
            self.current_step += 1
        ...
        obs = self._augment_observation() if not truncated else np.zeros(self.observation_dim)

        return obs, reward, False, truncated, {"is_success": hit}
