...
def load_worker_model(model_path: str, device: str = "cpu") -> SAC:
    """Worker is SAC (CPU) in this training variant."""
...
def evaluate_one_hrl_experiment(
    ...
    if eval_worker_too:
        ...
        worker_model = SAC.load(paths.worker_model, env=worker_env, device=worker_device)
...
def _parse_args():
    ...
    p.add_argument("--glob", type=str, default="*sac_hrl/", help="Glob under logs root to select experiments")
    ...
    p.add_argument("--worker-device", type=str, default="cpu", help="cpu/cuda/auto")
    p.add_argument("--manager-device", type=str, default="cpu", help="cpu/cuda/auto")
    return p.parse_args()
