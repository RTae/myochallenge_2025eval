...
def load_model(model_path: str, env, device: str = "auto") -> SAC:
    """
    Ensure SB3 can deserialize the custom policy.
    """
    return SAC.load(
        model_path,
        env=env,
...
def resolve_paths(exp_dir: str, use_best: bool) -> EvalPaths:
    """
    If use_best:
    ...
    model_path = (
        ...
        if use_best
        else os.path.join(exp_dir, "model.pkl")
    )

    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Missing required file: {model_path}")

    return EvalPaths(model_path=model_path)
...
def evaluate_one_experiment(
    ...
    paths = resolve_paths(exp_dir, use_best=use_best)

    env = make_eval_env(cfg)
    try:
        model = load_model(paths.model_path, env=env, device=device)
...
def _parse_args():
    ...
    p.add_argument("--glob", type=str, default="sac_lattice*/", help="Glob under logs root to select experiments")
    p.add_argument("--trials", type=int, default=1000)
    p.add_argument("--use-best", action="store_true", help="Use best_model/best_model.zip instead of model.pkl")
    p.add_argument("--device", type=str, default="auto", help="SB3 device for loading (auto/cpu/cuda)")
    return p.parse_args()
