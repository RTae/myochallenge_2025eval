"""
HRL SAC + LatticeSACPolicy EVAL script for the last training code you pasted.
...
Folder layout assumed:
  <exp_dir>/
    worker/
      worker_model.pkl
      vecnormalize.pkl
      best/best_model.zip   (optional)
    manager/
      manager_model.pkl
      best/best_model.zip   (optional)

This mirrors the structure/style of your “HRL eval” template:
- freeze_vecnormalize
- run_vecenv_episodes
- resolve_hrl_paths
- make_eval_worker_env / make_eval_manager_env
- evaluate_one_hrl_experiment
- evaluate_hrl_folders
- CLI

Important:
- Worker model is SAC with policy=LatticeSACPolicy, device=cuda (as in training).
- Manager model is plain SAC (MlpPolicy), device=cpu (as in training).
- VecNormalize stats are loaded from worker/vecnormalize.pkl.
"""
...
def load_worker_model(model_path: str, device: str = "cuda") -> SAC:
    """Worker is SAC with LatticeSACPolicy in this training variant."""
    return SAC.load(model_path, device=device, policy=LatticeSACPolicy)
...
def evaluate_one_hrl_experiment(
    ...
    # Manager is plain SAC (MlpPolicy) in your training code
    manager_model = SAC.load(paths.manager_model, env=manager_env, device=manager_device)
...
def _parse_args():
    ...
    p.add_argument("--worker-device", type=str, default="cuda", help="cuda/cpu/auto")
    p.add_argument("--manager-device", type=str, default="cpu", help="cuda/cpu/auto")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    evaluate_hrl_folders(
        ...
        manager_device=args.manager_device,
    )